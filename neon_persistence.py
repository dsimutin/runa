...
from database import delete_user_state, load_all_user_states, save_user_state
...
class NeonPersistence(BasePersistence[dict, dict, dict]):
    ...
    async def get_user_data(self) -> dict[int, dict[str, Any]]:
        return await asyncio.to_thread(load_all_user_states)

    async def refresh_user_data(self, user_id: int, user_data: dict[str, Any]) -> None:
        # A single Cloud Run instance owns the in-memory copy. It is loaded at
        # application startup and saved atomically with every processed update.
        return None

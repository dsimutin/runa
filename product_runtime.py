...
DAILY_OPENINGS = {
    "light": [
        "🌕 <b>{name}, руна дня</b>\nСегодня лучше действовать мягко и без лишнего нажима.",
        "🌕 <b>{name}, руна дня</b>\nДень про внимательность к состоянию и спокойный выбор.",
        "🌕 <b>{name}, руна дня</b>\nСейчас важнее не торопиться, а услышать, где внутри становится ровнее.",
        "🌕 <b>{name}, руна дня</b>\nЭта карта больше про бережный поворот, чем про резкий шаг.",
        "🌕 <b>{name}, руна дня</b>\nСегодня хорошо сработает простое: меньше давления, больше ясности в одном деле.",
    ],
    "dark": [
        "🌑 <b>{name}, руна дня</b>\nКарта показывает место, где лучше не обходить правду стороной.",
        "🌑 <b>{name}, руна дня</b>\nСегодня стоит отделить факт от удобного объяснения.",
        "🌑 <b>{name}, руна дня</b>\nЗнак про границу: где её пора обозначить спокойнее и твёрже.",
        "🌑 <b>{name}, руна дня</b>\nСитуация просит собранности, а не лишних оправданий.",
        "🌑 <b>{name}, руна дня</b>\nСегодня лучше смотреть прямо на то, что уже давно заметно.",
    ],
    "premium": [
        "💠 <b>{name}, руна дня</b>\nЗдесь важен не шум события, а скрытая связка между деталями.",
        "💠 <b>{name}, руна дня</b>\nКарта выводит на первый план то, что обычно остаётся между строк.",
        "💠 <b>{name}, руна дня</b>\nСегодня стоит смотреть глубже обычного: не на жест, а на мотив.",
        "💠 <b>{name}, руна дня</b>\nВ этой ситуации есть второй слой — именно он задаёт направление.",
        "💠 <b>{name}, руна дня</b>\nОбрати внимание не на обещания, а на то, как всё складывается на деле.",
    ],
}

DAILY_ACTIONS = {
    "light": [
        "Выбери один спокойный шаг и не пытайся закрыть всё сразу.",
        "Сделай проще: убери лишнее действие, разговор или ожидание.",
        "Дай себе время на решение, но не уходи от него полностью.",
        "Поддержи то, что уже даёт ощущение опоры.",
        "Начни с малого — сегодня этого достаточно.",
        "Не подстраивайся автоматически. Проверь, что тебе действительно подходит.",
        "Сохрани ресурс: не бери на себя чужую срочность.",
    ],
    "dark": [
        "Назови проблему точнее — без драматизации, но и без смягчения.",
        "Проверь, где ты уже уступаешь больше, чем хотел.",
        "Один честный разговор сегодня полезнее длинного ожидания.",
        "Убери лишний компромисс: он уже начал стоить дороже, чем кажется.",
        "Не доказывай позицию. Просто обозначь её.",
        "Смотри на поступки, а не на объяснения.",
        "Сохрани дистанцию там, где тебя пытаются втянуть в чужой хаос.",
    ],
    "premium": [
        "Отследи повтор: кто, где или какая мысль возвращается снова.",
        "Не отвечай сразу. Сначала пойми, какой мотив стоит за просьбой или событием.",
        "Сними один внутренний узел: ожидание, обещание или старую привязку.",
        "Смотри не на один эпизод, а на цепочку последних дней.",
        "Запиши главный вопрос коротко. Так станет видно, что в нём лишнее.",
        "Не расширяй ситуацию новыми действиями, пока не понятен её центр.",
        "Сегодня полезнее распутать причину, чем реагировать на поверхность.",
    ],
}

QUESTION_OPENINGS = {
    "light": [
        "❓ <b>{name}, ответ по смыслу «да/нет»</b>",
        "❓ <b>{name}, смотрю вопрос одной картой</b>",
        "❓ <b>{name}, здесь ответ мягкий, но читаемый</b>",
        "❓ <b>{name}, карта даёт короткий ориентир</b>",
        "❓ <b>{name}, ответ лучше читать спокойно</b>",
    ],
    "dark": [
        "❓ <b>{name}, ответ по смыслу «да/нет»</b>",
        "❓ <b>{name}, карта показывает сухой остаток</b>",
        "❓ <b>{name}, здесь лучше смотреть прямо</b>",
        "❓ <b>{name}, ответ не мягкий, но полезный</b>",
        "❓ <b>{name}, карта подсвечивает слабое место вопроса</b>",
    ],
    "premium": [
        "❓ <b>{name}, ответ по смыслу «да/нет»</b>",
        "❓ <b>{name}, карта читает подтекст вопроса</b>",
        "❓ <b>{name}, здесь важна скрытая причина</b>",
        "❓ <b>{name}, ответ идёт через второй слой ситуации</b>",
        "❓ <b>{name}, карта показывает не только событие, но и мотив</b>",
    ],
}

QUESTION_FINALS = {
    "light": [
        "Выбирай тот шаг, после которого внутри становится спокойнее.",
        "Не усложняй. Сейчас достаточно одного ясного действия.",
        "Лучше проверить ощущение ещё раз, чем идти из тревоги.",
        "Оставь себе право на паузу, но не превращай её в бегство.",
        "Если ответ вызывает облегчение — это важный знак.",
        "Сейчас поможет не контроль, а честность с собой.",
        "Пусть решение будет маленьким, но твоим.",
    ],
    "dark": [
        "Оцени не слова, а последствия ближайшего шага.",
        "Не соглашайся только ради мира. Цена может оказаться выше.",
        "Если граница уже нарушена, её придётся вернуть.",
        "Сомнение здесь не случайно. Проверь, что именно тебя насторожило.",
        "Не торгуйся с тем, что давно стало очевидным.",
        "Лучше короткая ясность, чем длинная неопределённость.",
        "Не бери ответственность за чужой выбор.",
    ],
    "premium": [
        "Смотри на повторяющийся сценарий, а не только на текущий эпизод.",
        "Вопрос стоит переформулировать: тогда станет видно, где настоящий центр.",
        "Ответ проявится через то, что повторится в ближайшие дни.",
        "Не цепляйся за форму. Смотри, какой мотив за ней стоит.",
        "Здесь лучше не ускорять итог, а понять скрытую связку.",
        "Если ситуация кажется запутанной, ищи не больше фактов, а главный узел.",
        "Проверь, не пытаешься ли ты получить разрешение на то, что уже чувствуешь.",
    ],
}
...
def onboarding_keyboard(step: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("A", callback_data=f"onboarding:{step}:light")], [InlineKeyboardButton("B", callback_data=f"onboarding:{step}:dark")], [InlineKeyboardButton("C", callback_data=f"onboarding:{step}:premium")]]
    )
...
def product_daily_text(name: str, main: dict, main_text: dict, aux: dict, aux_text: dict, palette: str, main_alt: bool, aux_alt: bool) -> str:
    key = palette if palette in DAILY_OPENINGS else "light"
    opening = stable_pick(DAILY_OPENINGS[key], name, main["key"], aux["key"], date.today().isoformat()).format(name=name)
    action = stable_pick(DAILY_ACTIONS[key], name, main["key"], aux["key"], "action", date.today().isoformat())
    ...
    return f"{opening}\n\n<u>Основная руна</u>\n{rune_title(main, main_alt)}\n\n{main_desc}\n\n<u>Дополнительная руна</u>\n{rune_title(aux, aux_alt)}\n\n{aux_desc}\n\n<u>Что сделать сегодня</u>\n{action}"


def product_question_text(name: str, question: str, rune: dict, answer: str, palette: str, alt: bool) -> str:
    key = palette if palette in QUESTION_OPENINGS else "light"
    opening = stable_pick(QUESTION_OPENINGS[key], name, question, rune["key"], alt).format(name=name)
    final = stable_pick(QUESTION_FINALS[key], name, question, rune["key"], alt, "final")
